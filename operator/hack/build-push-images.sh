...
docker build -t "${REGISTRY}/kaos-agent:${AGENT_TAG}" "${PROJECT_ROOT}/data-plane/kaos-server/"
...
kind load docker-image "${REGISTRY}/kaos-mcp-pctx:${AGENT_TAG}" --name "${KIND_CLUSTER_NAME}"
