    ...
    async with httpx.AsyncClient(timeout=10.0) as client:
        for _ in range(max_wait):
            try:
                ...
                # - 405: Method not allowed (pctx-codemode uses POST-only for /mcp)
    ...
    try:
        ...
        helm_args.extend(
            [
                ...
                "--set",
                "gateway.defaultTimeouts.agent=120s",
                "--set",
                "gateway.defaultTimeouts.modelAPI=120s",
                "--set",
                "gateway.defaultTimeouts.mcp=30s",
                "--skip-crds",
                "--wait",
                "--timeout",
                "120s",
