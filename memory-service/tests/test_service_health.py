...
def _client(longterm, working) -> TestClient:
...
def test_readyz_ok_when_both_stores_reachable():
    ...
    assert body["stores"] == {"working": True, "longterm": True}


def test_readyz_503_when_a_store_is_unreachable():
    client = _client(longterm=_BrokenStore(), working=_OkStore())
    resp = client.get("/readyz")
    assert resp.status_code == 503
    body = resp.json()
    assert body["ready"] is False
    assert body["stores"]["working"] is True
