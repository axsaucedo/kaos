...
- **The short-term window is session-scoped only.** Every window and digest key combines the selected scope owner with the session id; a missing session id fails rather than falling back to a shared owner window. User-, agent-, or store-wide verbatim windows would interleave concurrent conversations, so cross-session continuity is served by long-term facts instead of merging live turns. In particular, `scope: group` does not share raw turns across agents; cross-agent sharing flows through extracted long-term facts. The window is bounded by a configurable **token budget** (with a hard event-count safety cap), not by turn count.
...
- **Conversational tiers are always per-session.** The verbatim window and medium-term digest use `<owner key>:<owner value>|run:<session id>`. `scope: user` does not create a user-wide conversational window. Session forget deletes one composite key; user, agent, and group forget delete every session under the exact owner prefix.
