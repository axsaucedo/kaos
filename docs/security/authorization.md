...
The protected-resource hostname is the Agent-facing egress name and must resolve to the gateway. When the real upstream uses a different origin, annotate that hostname's namespace-local Kubernetes `Service` with `kaos.tools/token-exchange-upstream-origin: https://api.example.com`. The generated `HTTPRoute` continues to match the protected-resource hostname while its generated `Backend` uses the annotated HTTP(S) origin, preventing a gateway loop. The annotation accepts an origin only, without a path, query, credentials, or fragment.
