...
The gateway's agent JWT provider validates the token against the selected issuer and requires audience `kaos-gateway`. ServiceAccount mode uses the discovered Kubernetes issuer and an inline JWKS. AIB mode verifies the exact issuer through RFC 8414 `/.well-known/oauth-authorization-server` discovery and uses its advertised `jwks_uri`; the broker must mint agent tokens with `kaos-gateway` in their audience claim.

## Delegated third-party token exchange

The operator retains the Keycloak-backed token-exchange projection for agents acting as a user against an external OAuth service. The public AIB v0.1.8 chart does not deploy extProc, so the CLI deploys the published extProc image as a separate fail-closed workload when `--token-exchange-enabled` is selected.
...
For identity-only installs, the CLI seeds exactly one placeholder service and one `kaos-identity` permission set because AIB v0.1.8 rejects agents without a permission-set binding. The operator resolves that set by name, attaches it as mandatory, and fails static with an Agent condition when it is missing. The operator does not create or manage other AIB authorization data.

The runtime re-mint is implemented separately. Its contract is an `Authorization: Bearer <token>` header containing a Keycloak token with the original user `sub`, the acting Agent's DCR client id in `azp`, and `token-exchange-broker` in `aud`. The PDP requires that `azp` to match the verified actor's projected DCR client id, so unbound or non-reminted egress is denied before ext_proc. The chart orders gateway filters as `jwt_authn`, the PDP `ext_authz`, an operator-generated Lua metadata producer, and then `ext_proc`. Lua publishes the validated raw token and exact request resource URI under `aib.tokenexchange`; ext_proc receives only that namespace, remains fail closed, and replaces `Authorization` only on the dedicated third-party route.
