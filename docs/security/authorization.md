...
The operator retains the Keycloak-backed token-exchange projection for agents acting as a user against an external OAuth service. The public AIB v0.1.8 chart does not deploy extProc, so the CLI now rejects `--token-exchange-enabled` with a clear error until that sidecar is deployed separately. Full token-exchange installation is deferred; static per-MCP credentials remain the default.
