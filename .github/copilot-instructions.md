...
- `/issue-scout`: Invoke with no arguments (typically from a daily routine) — scans the repo through one date-rotated research lens, applies a strict quality bar, and files at most one evidence-backed GitHub issue; files nothing on days with no strong candidate
