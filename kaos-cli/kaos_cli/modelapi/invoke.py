...
def invoke_command(
    ...
    result = subprocess.run(
        ["kubectl", "get", "svc", f"modelapi-{name}", "-n", namespace, "-o", "jsonpath={.spec.ports[0].port}"],
        capture_output=True,
        text=True,
    )
    
    if result.returncode != 0:
        typer.echo(f"Error: ModelAPI '{name}' not found in namespace '{namespace}'", err=True)
        sys.exit(1)
    ...
    pf_process = subprocess.Popen(
        ["kubectl", "port-forward", f"svc/modelapi-{name}", f"{port}:{svc_port}", "-n", namespace],
