...
def runtimes_command(namespace: str) -> None:
    ...
    typer.echo(f"Available MCP Runtimes (from kaos-mcp-runtimes ConfigMap in {namespace})")
    typer.echo("=" * 60)
    
    try:
        result = subprocess.run(
            ["kubectl", "get", "configmap", "kaos-mcp-runtimes", "-n", namespace, "-o", "jsonpath={.data.runtimes\\.yaml}"],
            capture_output=True,
            text=True,
        )
